...
class NsBasethemeUtility
{
     ...
     * @return array
     **/
    public function getChildThemeComponents($arrAllExtensions): array
    {
        $allComponents = [];

        // Get default components from EXT:ns_basetheme
        $arrAllComponents['ns_basetheme'] = scandir($this->getDirectoryPath('ns_basetheme', 'Configuration/FlexForms'));
            ...
            foreach ($arrAllExtensions as $key => $extKey) {
                ...
                if ($extensionPrefixKey == 'ns_theme_') {
                    if(\TYPO3\CMS\Core\Core\Environment::isComposerMode()) {
                        $extPackageName = str_replace('_','-',$extKey);
                        $directoryPath =  \TYPO3\CMS\Core\Core\Environment::getProjectPath() . "/vendor/nitsan/$extPackageName/Configuration/FlexForms";
                    }
                    else{
        ...
        if (count($arrAllComponents) > 0) {
            foreach ($arrAllComponents as $extKey => $extValue) {
                if (is_array($extValue) && count($extValue) > 0) {
                    foreach ($extValue as $key => $value) {
                        if ($value != '.' && $value != '..' && str_contains($value, '.xml')) {
