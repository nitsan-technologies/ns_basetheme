...
class NsBasethemeRepository extends \TYPO3\CMS\Extbase\Persistence\Repository
{
    ...
    {
        $queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('tx_nsbasetheme_domain_model_apidata');
        $queryBuilder
            ->select('*')
            ->from('tx_nsbasetheme_domain_model_apidata');
    ...
    public function insertNewData($data)
    {
        $queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('tx_nsbasetheme_domain_model_apidata');
        $row = $queryBuilder
            ->insert('tx_nsbasetheme_domain_model_apidata')
            ->values($data);
    ...
    public function deleteOldApiData()
    {
        $queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('tx_nsbasetheme_domain_model_apidata');
        $queryBuilder
            ->delete('tx_nsbasetheme_domain_model_apidata')
            ->where(
